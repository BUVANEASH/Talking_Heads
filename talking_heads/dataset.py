...
def preprocess_dataset(source, output, device='cpu', size=0, overwrite=False):
    ...
    * Select K+1 frames of each video that will be kept. K frames will be used to train the embedder network, while
    the other one will be used to train the generator network. The value of K can be configured in the hyperparams.py file.
    ...
    * The frames and the corresponding landmarks for each video will be saved in files (one for each video) in the
    ...
    logging.info('Running on {}'.format(device.upper()))
    logging.info('Saving K+1 random frames from each video (K = {}).'.format(hp.K))
    fa = FaceAlignment(LandmarksType._2D, device=device)

    video_list = get_video_list(source, size, output, overwrite=overwrite)

    logging.info('Processing {} videos...'.format(len(video_list)))
    ...
    for v in video_list:
        ...
        logging.info('{0}/{1}'.format(counter,len(video_list)))
        counter += 1

    logging.info('All {} videos processed.'.format(len(video_list)))
...
def get_total_video_list(source):
    """
    Extracts a list of paths to videos to pre-process during the current run.
...
def process_video_folder(video):
    ...
    try:
        ...
        logging.error('Video {0} could not be processed:\n{1}'.format(os.path.basename(os.path.normpath(folder)),e))
...
def plot_landmarks(frame, landmarks):
    ...
    * The axis must be turned off and the subplot must be adjusted to remove the space where the axis would normally be.
    ...
    # Head
    ax.plot(landmarks[:17, 0], landmarks[:17, 1], linestyle='-', color='green', lw=2)
    # Eyebrows
    ...
    ax.plot(landmarks[60:, 0], landmarks[60:, 1], linestyle='-', color='purple', lw=2)
